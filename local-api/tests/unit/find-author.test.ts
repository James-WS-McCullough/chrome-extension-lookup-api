  ...
  samplePayloads: [
    {
      ...
      value: "Start with a minimal viable flow.",
    },
...
const TEST_AUTHORS: Author[] = [albertEinstein, jkRowling];
...
describe("FindAuthorUseCase", () => {
  ...
  it("returns author for exact match", () => {
    ...
    expect(result).toEqual(albertEinstein);
  });
  ...
  it("returns correct author when multiple exist", () => {
    const result = useCase.execute("J.K. Rowling");
    expect(result).toEqual(jkRowling);
  });
